...
// ─── ActionRow ───────────────────────────────────────────────────────────────
function ActionRow({ action }: { action: HandAction }) {
...
function EditableActionRow({ action, onUpdate, onRemove }: {
    action: HandAction;
...
// ─── StreetBlock ─────────────────────────────────────────────────────────────
function StreetBlock({ label, pot, boardCards, actions, editable, onSetEditingCard, boardStartIndex, handData, parsedHand, setParsedHand, streetKey }: {
    label: string;
    pot?: number;
    boardCards: string[];
    actions: HandAction[];
    ...
    return (
        <div className="border border-gray-700 hover:border-gray-500 transition-colors rounded-md px-3 py-2 min-h-[120px] bg-[#0f1115]">
            ...
            <div className="flex items-center gap-2 border-b border-gray-800 pb-1 mb-1 text-yellow-400">
                ...
                {pot != null && pot > 0 && (
                    <span className="text-gray-400 text-xs">({pot.toLocaleString()} BB)</span>
                )}
                {boardCards.length > 0 && (
...
function SaveNoteButton({ noteData }: { noteData: any }) {
    ...
    const handle = async (e: React.MouseEvent) => {
        ...
        try { await createNote(noteData); setSaved(true); } catch { alert("Save failed"); }
        finally { setSaving(false); }
    };
    if (saved) return <span className="text-green-500 text-xs">✓</span>;
    return <button onClick={handle} disabled={saving} className="text-gray-600 hover:text-yellow-400 text-xs">{saving ? "..." : "save"}</button>;
}
...
export function HandAnalyzer() {
    ...
    // ── SKELETON MOCK (remove after review) ──
    const MOCK_HAND: Hand = {
        ...
        parsed_data: {
            ...
            actions: {
                preflop: [
                    { player: "BigManTing", action: "fold", position: "UTG1" },
                    { player: "Lethanh92", action: "fold", position: "MP" },
                    { player: "天涯海角2", action: "fold", position: "HJ" },
                    { player: "kiukiukiukiu902", action: "raise", amount: 7.5, position: "CO" },
                    { player: "Vipbka1", action: "call", amount: 7.5, position: "BTN" },
                    { player: "全部刮过来113", action: "fold", position: "SB" },
                    { player: "chipboiz", action: "call", amount: 7.5, position: "BB" },
                ],
                flop: [
                    { player: "chipboiz", action: "bet", amount: 37.3, position: "BB" },
                    { player: "kiukiukiukiu902", action: "call", amount: 37.3, position: "CO" },
                    { player: "Vipbka1", action: "raise", amount: 111, position: "BTN" },
                ],
                turn: [
                    { player: "chipboiz", action: "check", position: "BB" },
                    { player: "Vipbka1", action: "bet", amount: 2, position: "BTN" },
                    { player: "kiukiukiukiu902", action: "fold", position: "CO" },
                ],
                river: [
                    { player: "kiukiukiukiu902", action: "all-in", amount: 1128, position: "CO" },
                    { player: "Vipbka1", action: "fold", position: "BTN" },
                    { player: "chipboiz", action: "fold", position: "BB" },
                ],
            },
            pot: 1947,
            winner: "kiukiukiukiu902",
        } as any,
    };
    (MOCK_HAND.parsed_data as any).street_pots = { preflop: 7.5, flop: 113, turn: 1947, river: 1947 };
    (MOCK_HAND.parsed_data as any).actions.blinds_ante = [
        { player: "Cước Ante", action: "post", amount: 4 },
        { player: "全部刮过来113", action: "post", amount: 0.5, position: "SB" },
        { player: "chipboiz", action: "post", amount: 1, position: "BB" },
    ];

    const [parsedHand, setParsedHand] = useState<Hand | null>(MOCK_HAND);
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                            ...
                            <div className="flex flex-col gap-3">
                                {streetDefs.slice(0, 2).map(({ key, label, cards, boardStart }) => (
                                    <StreetBlock key={key} streetKey={key} label={label} pot={handData.street_pots?.[key]} boardCards={cards} actions={handData.actions?.[key] || []} editable={editable} onSetEditingCard={setEditingCard} boardStartIndex={boardStart} handData={handData} parsedHand={parsedHand} setParsedHand={setParsedHand} />
                                ))}
                            </div>
                            {/* Column 2: Flop + Turn */}
                            <div className="flex flex-col gap-3">
